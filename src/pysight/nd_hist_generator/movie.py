...
class Movie:
    ...
    name = attr.ib(default="Movie", validator=instance_of(str), converter=trunc_end_of_file)
    ...
    def __determine_outputs(self) -> Tuple[List[Callable], List[Callable]]:
        ...
        """
        if not self.outputs:
            logging.warning("No outputs requested. Data is still accessible using the dataframe variable.")
            return [], []
    ...
    def __process_data(self, funcs_during: List[Callable], funcs_end: List[Callable]) -> None:
        """
        ...
        tq = tqdm(total=self.num_of_frame_chunks, desc=f"Processing frame chunks...", unit="chunk", leave=False,)
        for idx, frame_chunk in enumerate(self.frame_slices):
            sliced_df_dict, num_of_frames, frames, lines = self.__slice_df(frame_chunk)
            chunk = FrameChunk(
                movie=self, df_dict=sliced_df_dict, frames_per_chunk=num_of_frames, frames=frames, lines=lines,
            )
    ...
    def __slice_df(self, frame_chunk) -> Tuple[Dict[int, pd.DataFrame], int, pd.Series, pd.Series]:
        ...
        if len(lines) > self.x_pixels * num_of_frames:
            logging.warning(f"More-than-necessary line signals in the frame of chunk {frame_chunk}.")
...
class HistWithIndex:
    ...
    def _get_indices_for_photons(self):
        ...
        """
        indices = tuple(np.searchsorted(edges, self.data[idx], side="right") for idx, edges in enumerate(self.edges))
...
class FlimCalc:
    ...
    def _partition_photons_into_bins(self):
        ...
        """
        self.hist_arrivals = self.all_data.groupby("bin", as_index=False, sort=False).agg(calc_lifetime)
...
def calc_lifetime(data: pd.Series, bins_bet_pulses=125) -> float:
    ...
    popt, _ = curve_fit(
        _exp_decay, np.arange(len(decay_curve)), decay_curve, p0=(max_val, 1 / 35, min_val), maxfev=10_000,
    )
    return 1 / popt[1]
...
def find_decay_borders(hist: np.ndarray, peaks: np.ndarray, props: Dict[str, np.ndarray]):
