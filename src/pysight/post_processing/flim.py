...
def add_downsample_frame_idx_to_df(data: pd.DataFrame, chan: int, frames: pd.Series, num_of_frames=256):
    """
    ...
    if (len(frames) // num_of_frames > 0 and len(frames) % num_of_frames > 0):
        data.loc[pd.IndexSlice[chan, end_frame:], 'frame_idx'] = len(frames) // num_of_frames
...
def flip_photons(data: pd.DataFrame, edges_of_lines_dim: np.array, lines: np.array, num_of_pixels: int):
    """
...
def calc_lifetime(data, bins_bet_pulses=125) -> float:
    ...
    # estimate tau
    try:
        popt, _ = curve_fit(_exp_decay, np.arange(len(sg_hist)), sg_hist,
