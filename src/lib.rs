...
/// Find the type of string that ends each line if it's an ascii file. Can be
/// "\n", "\r" or "\r\n"
fn find_end_of_line(data: &[u8], start_of_data: usize, timepatch: &str) -> Option<String> {
    let tpbits: u8 = TimepatchBits::new(timepatch).iter().sum();
    let start_of_eol = ((tpbits + 4u8) * 2u8) as usize + start_of_data;
    let eol_slice: &[u8] = &data[start_of_eol..start_of_eol + 2];
    let eol_str = str::from_utf8(eol_slice).unwrap();

    match &eol_str[1..] {
        "\n" => Some(eol_str.to_string()),
        _ => Some(eol_str[..1].to_string()),
    }
...
pub fn analyze_lst(fname: &str, start_of_data: usize, range: u64,
                   timepatch: &str, channel_map: Vec<u8>, file_type: &str)
    -> Result<HashMap<u8, Vec<DataLine>>, Error> {
    ...
    let mut linesep = None;
    if file_type == "ascii" {
        linesep = find_end_of_line(&data_with_headers, start_of_data, timepatch);
    }
    let mut data: &[u8];
    let data_vec;
    ...
    match file_type {
        "binary" => {
            data = &data_with_headers[start_of_data..];
        },
        "ascii" => {
            let data_as_str = unsafe {
                str::from_utf8_unchecked(&data_with_headers[start_of_data..])
            };
            let mut spec = HEXLOWER.specification();
            spec.ignore.push_str(&linesep.unwrap());
            let enc = spec.encoding().expect("Encoding error.");
            data_vec = enc.decode(&data_with_headers[start_of_data..]).expect("Decoding error.");
        },
