...
class TestDeinterleave:
    ...
    df = (pd.DataFrame({'time_rel_pulse': photons,
                        'abs_time': abs_time,
         ...
         .set_index(['Channel', 'Lines', 'Frames'],
                   drop=True))
    ...
    def test_correct_allocation_early(self):
        ...
        assert np.array_equal(early_photons['time_rel_pulse'].values.ravel(), np.array([1, 5, 6, 4, 5, 3], dtype=np.uint8))

    def test_correct_allocation_late(self):
        idx_slice = pd.IndexSlice
        late_photons = self.deinter_photons.loc[idx_slice[7, :, :], :].copy()
        assert np.array_equal(late_photons['time_rel_pulse'].values.ravel(), np.array([15, 11, 15, 11, 9], dtype=np.uint8))

    def test_abs_time_unchanged(self):
        assert np.array_equal(self.deinter_photons['abs_time'].values.ravel().sort(), self.abs_time.sort())
