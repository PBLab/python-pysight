...
class TestLineRectifier(TestCase):
    def test_normal_line_signal_from_zero_unidir(self):
        ...
        rect = LineRectifier(lines=raw_lines[:-1], x_pixels=num_lines, bidir=False)
    ...
    def test_normal_line_signal_from_zero_bidir(self):
        ...
        raw_lines = np.arange(start=0, stop=100, step=10, dtype=np.uint64)
        num_lines = len(raw_lines[:-1])
        rect = LineRectifier(lines=raw_lines[:-1], x_pixels=num_lines, bidir=True)
    ...
    def test_normal_line_signal_with_noise_unidir(self):
        ...
        raw_lines += ((np.random.rand(raw_lines.shape[0]) - 0.5) * step / 25).astype(
            np.uint64
        )
        num_lines = len(raw_lines[:-1])
        rect = LineRectifier(lines=raw_lines[:-1], x_pixels=num_lines, bidir=False)
    ...
    def test_missing_lines_from_end_unidir(self):
        ...
        num_lines = len(raw_lines[:-1])
        del_lines = np.delete(
            raw_lines, [len(raw_lines) - 2, len(raw_lines) - 3, len(raw_lines) - 4]
        )
        rect = LineRectifier(lines=del_lines[:-1], x_pixels=num_lines, bidir=False)
        self.assertSequenceEqual(list(rect.rectify()), list(raw_lines))
    ...
    def test_missing_head_tail_unidir(self):
        ...
        num_lines = len(raw_lines[:-1])
        del_lines = np.delete(
            raw_lines, [0, 1, 2, 3, len(raw_lines) - 2, len(raw_lines) - 3]
        )
        rect = LineRectifier(lines=del_lines[:-1], x_pixels=num_lines, bidir=False)
