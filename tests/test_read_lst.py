...
class TestReadLst:
    ...
    timepatch = "32"
    is_binary = False
    readlst_hex = read_lst.ReadData(filename=filename,
                                    start_of_data_pos=start_of_data_pos,
                                    timepatch=timepatch,
                                    is_binary=False,
                                    debug=False)
    readlst_bin = read_lst.ReadData(filename=filename,
                                    start_of_data_pos=start_of_data_pos,
                                    timepatch=timepatch,
                                    is_binary=True,
                                    debug=False)
    
    @pytest.mark.parametrize("tp, bytess", [
        ("0", 6),
        ("5", 10),
        ("1", 10),
        ("1a", 14),
    ...
    def test_ascii_readout(self, data_length=14, num_of_lines=-1):
        data = np.array(['0100000060d9\r\n', '010000008289\r\n', '010000009099\r\n',
            '01000000a3a9\r\n', '0100000144f9\r\n', '010000018939\r\n',
            '01000001d139\r\n', '01000001e8b9\r\n'], dtype='<U14')
        assert np.setdiff1d(data, self.readlst_hex._read_ascii(data_length, num_of_lines)).shape == (0,)